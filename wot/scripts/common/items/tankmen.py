...
# Compiled at: 2015-04-30 06:57:10
...
class TankmanDescr(object):
    ...
    def lastSkillLevel(self):
        return self.__lastSkillLevel if self.freeSkillsNumber < len(self.__skills) else MAX_SKILL_LEVEL
...
def _readRanks(xmlCtx, subsections):
    ...
    for sname, subsection in subsections:
        ...
        if not (IS_CLIENT or IS_WEB):
            ranks.append(None)
        ranks.append({'userString': 'rankPlaceholder',#i18n.makeString(_xml.readNonEmptyString(ctx, subsection, 'userString')),
