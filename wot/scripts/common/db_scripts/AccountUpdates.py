...
from db_scripts.handlers.ShopHandler import premiumPrices, creditsPrice, slotsPrices, freeXPRate, crewRecruitLevelsAndPrices
...
def __buyVehicle(s_data, i_data, shopRev, vehTypeCompDescr, flags, crew_level, int3):
	...
	if vehicle_prices['itemPrices'].get(vehTypeCompDescr) and vehTypeCompDescr not in notInShopItems:
		...
		vehicle_price = list(vehicle_prices['itemPrices'].get(vehTypeCompDescr))
	...
	if flags & AccountCommands.BUY_VEHICLE_FLAG.CREW:
		...
		tmanList = items.tankmen.generateTankmen(value[0], value[1], vehicle.type.crewRoles, True, crewLevel, [])
		tmanListCycle = cycle(tmanList)
		
		i_data['inventory'][ITEM_TYPE_INDICES['vehicle']]['crew'].update({i: [tmanID for tmanID in xrange(i_crew, len(tmanList) + i_crew)]})
...
def __addPremiumTime(extend_by_days, data, use_gold=True):
	...
	if attrs & ACCOUNT_ATTR.PREMIUM:
		...
		if use_gold: data['stats']['gold'] -= pcost
		if data['account'][
			'premiumExpiryTime'] < current_epoch:  # they have had premium in the past; and we have no functionality to set it back to zero after it expires.
			data['account']['premiumExpiryTime'] = (extend_by_days * 86400) + current_epoch
		else:
			data['account']['premiumExpiryTime'] += (extend_by_days * 86400) + current_epoch
		data['account']['attrs'] |= ACCOUNT_ATTR.PREMIUM
