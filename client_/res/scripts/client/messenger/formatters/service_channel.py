...
# Compiled at: 2015-03-25 08:59:46
import types
...
class BattleResultsFormatter(ServiceChannelFormatter):
    ...
    def __makeVehicleLockString(self, vehicle, battleResult):
        ...
        return '' if not expireTime else g_settings.htmlTemplates.format('battleResultLocks', ctx={'vehicleName': vehicle,
...
class InvoiceReceivedFormatter(ServiceChannelFormatter):
    ...
    def __formatAmount(self, assetType, data):
        ...
        return None if amount is None else g_settings.msgTemplates.format(self.__messageTemplateKeys[assetType], ctx={'at': self.__getOperationTimeString(data),
    ...
    def __formatData(self, assetType, data):
        ...
        if dataEx is None or not len(dataEx):
            ...
            return None if not operations else g_settings.msgTemplates.format(self.__messageTemplateKeys[assetType], ctx={'at': self.__getOperationTimeString(data),
             'desc': self.__getL10nDescription(data),
    ...
    def format(self, message, *args):
        ...
        return (formatted, self._getGuiSettings(message, self.__messageTemplateKeys[assetType])) if formatted is not None else (None, None)
