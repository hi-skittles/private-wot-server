...
# Compiled at: 2015-03-31 12:13:59
...
class _LadderInfo(namedtuple('_LadderInfo', ['division',
 ...
 'rating'])):
    ...
    def getDivision(self):
        return self.division if self.isInLadder() else None

    def getRatingPoints(self):
        return ladderRatingLocal(self.rating, self.division) if self.isInLadder() else 0

    def getGlobalRatingPoints(self):
        return ladderRating(self.rating)

    def getLeague(self):
        return getLeagueByDivision(self.getDivision()) if self.isInLadder() else None
...
class _ClubInvitation(object):
    ...
    def __cmp__(self, other):
        ...
        return res if res else self._timestamp - other.getTimestamp()
...
class Club(object):
    ...
    def getMember(self, memberDbID=None):
        ...
        return self.__makeMemberItem(memberDbID, self.__clubDescr.members[memberDbID]) if self.hasMember(memberDbID) else None
    ...
    def getInvitation(self, invID):
        ...
        return invites[invID] if invID in invites else self.getApplicants().get(invID)
    ...
    def getPermissions(self, memberDbID=None):
        ...
        return MemberPermissions(self.__clubDescr.members[memberDbID]) if self.hasMember(memberDbID) else DefaultMemberPermissions()

    def getUnitInfo(self):
        from gui.clubs.club_helpers import getClubUnit
        unit = getClubUnit(self.__clubDbID)
        return _UnitInfo(*unit) if unit is not None else None
