...
# Compiled at: 2015-03-30 03:31:37
...
class _UnitActionValidator(object):
    ...
    def canCreatorDoAction(self, pInfo, stats, state, vInfo):
        ...
        return (valid, restriction) if not valid else self._validateSlots(stats, state)
    ...
    def _validateSlots(self, stats, state):
        return (False, UNIT_RESTRICTION.NOT_READY_IN_SLOTS) if stats.readyCount != stats.occupiedSlotsCount else (True, UNIT_RESTRICTION.UNDEFINED)
...
class ClubsActionValidator(_UnitActionValidator):
    ...
    def _validateSlots(self, stats, state):
        ...
        if unit.isRated() and self._rosterSettings.getMinSlots() > stats.occupiedSlotsCount:
            return (False, UNIT_RESTRICTION.MIN_SLOTS)
        return (False, UNIT_RESTRICTION.NOT_READY_IN_SLOTS) if not state.isInPreArena() and stats.readyCount != stats.occupiedSlotsCount else (True, UNIT_RESTRICTION.UNDEFINED)
