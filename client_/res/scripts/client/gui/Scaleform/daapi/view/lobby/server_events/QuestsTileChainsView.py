...
# Compiled at: 2015-03-12 09:07:13
import weakref
import operator
...
class QuestsTileChainsView(QuestsTileChainsViewMeta, AppRef):
    ...
    def __getCurrentFilters(self):
        return self._navInfo.potapov.filters if self._navInfo.potapov.filters is not None else (-1, QUEST_TASK_FILTERS_TYPES.ALL, False)
    ...
    def __getQuestStatusData(self, quest):
        ...
        if not quest.isUnlocked():
            ...
            return (_getText(TextType.STATUS_INFO_TEXT, _ms(QUESTS.TILECHAINSVIEW_TASKTYPE_COMPLETED_TEXT)), RES_ICONS.MAPS_ICONS_LIBRARY_FORTIFICATION_CHECKMARK) if quest.isMainCompleted() else (_getText(TextType.MAIN_TEXT, ''), None)
    ...
    def __getChainUserName(self, chainID):
        ...
        return _ms('#quests:tileChainsView/chainName/%s' % vehType) if vehType is not None else ''
