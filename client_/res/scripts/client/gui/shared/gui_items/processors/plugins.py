...
# Compiled at: 2014-12-22 09:16:03
...
class VehicleSellValidator(SyncValidator):
    ...
    def _validate(self):
        return makeError('vehicle_cannot_be_sold') if self.vehicle._checkForTags(VEHICLE_TAGS.CANNOT_BE_SOLD) else makeSuccess()
...
class PotapovQuestSlotsValidator(SyncValidator):
    ...
    def _validate(self):
        return makeError('NOT_ENOUGH_SLOTS') if not g_eventsCache.questsProgress.getPotapovQuestsFreeSlots() else makeSuccess()
