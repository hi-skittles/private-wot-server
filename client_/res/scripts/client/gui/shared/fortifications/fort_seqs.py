...
# Compiled at: 2015-03-18 04:31:11
import random
import time
import weakref
import calendar
...
class BattleItem(object):
    ...
    def getRoundStartTime(self):
        return self.itemData.roundStart if self.itemData.roundStart else self.getAttackTime()
...
class IClanFortInfo(object):
    ...
    def getDefencePeriod(self):
        return (time_utils.getTimeTodayForUTC(hour=self.getStartDefHour()), time_utils.getTimeTodayForUTC(hour=self.getFinishDefHour())) if self.getStartDefHour() != NOT_ACTIVATED else (None, None)
...
class PublicInfoItem(IClanFortInfo):
    ...
    def filter(self, filterType, favorites):
        return False if filterType == FORT_SCOUTING_DATA_FILTER.ELECT and self.getClanDBID() not in favorites else True
    ...
    def getTimeNewDefHour(self):
        return time_utils.getTimestampFromUTC(self.itemData.defHourChangeDay.timetuple()) if self.itemData.defHourChangeDay is not None else None
...
class ClanCardItem(IClanFortInfo):
    ...
    def upcomingAttack(self):
        return self.__upcomingAttacks[0] if self.__upcomingAttacks else None

    @property
    def closestAttackInCooldown(self):
        return self.__attacksInCooldown[-1] if self.__attacksInCooldown else None

    @property
    def closestDefenseInCooldown(self):
        return self.__defencesInCooldown[-1] if self.__defencesInCooldown else None
    ...
    def getOffDay(self):
        return self.itemData.offDay if self.itemData.offDay != NOT_ACTIVATED else NOT_ACTIVATED
    ...
    def getListScheduledAttacksAt(self, start, end):
        ...
        def filterThisDay(item):
            ...
            return True if start <= timestamp < end else False
