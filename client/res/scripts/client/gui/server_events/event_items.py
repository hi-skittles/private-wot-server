...
# Compiled at: 2015-03-19 09:59:41
import operator
import time
...
class HistoricalBattle(ServerEventAbstract):
    ...
    def getDescriptionUrl(self):
        ...
        return histNote % {'langID': getClientLanguage()} if histNote is not None else None
    ...
    def getShellsLayout(self, intCD):
        ...
        return tuple() if vehicleData is None else map(lambda data: (g_itemsCache.items.getItemByCD(data[0]), data[1]), AmmoIterator(vehicleData['ammoList']))
        ...
        if vehicleData is None:
            ...
            def calculateLayout(isBoughtForCredits):
                ...
                for shellCompDescr, count in AmmoIterator(shellsLayout):
                    ...
                    if shell.buyPrice[1] and isBoughtForCredits:
    ...
    def getShellsLayoutFormatedPrice(self, intCD, colorManager, checkMoney=True, joinString=False):
        ...
        return i18n.makeString('#historical_battles:ammoPreset/priceConcat').join(result) if joinString else result
...
def createQuest(questType, qID, data, progress=None, expiryTime=None):
    return PersonalQuest(qID, data, progress, expiryTime) if questType == constants.EVENT_TYPE.PERSONAL_QUEST else Quest(qID, data, progress)
