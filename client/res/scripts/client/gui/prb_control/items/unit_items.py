...
# Compiled at: 2015-03-20 07:37:09
...
class PlayerUnitInfo(object):
    ...
    def isInSearch(self):
        return self.unit.getState() & UNIT_STATE.IN_SEARCH > 0 if self.unit is not None else False

    def isInQueue(self):
        return self.unit.getState() & UNIT_STATE.IN_QUEUE > 0 if self.unit is not None else False

    def isInPreArena(self):
        return self.unit.getState() & UNIT_STATE.IN_PRE_ARENA > 0 if self.unit is not None else False
