...
class FittingItem(object):
    ...
    def isRemovable(self):
        return self.descriptor['removable'] if self.type == ITEM_TYPE_NAMES[9] else True
    ...
    def hasTurrets(self):
        ...
        """
        return len(self.descriptor.hull['fakeTurrets']['lobby']) != len(self.descriptor.type.turrets) if self.itemTypeName == ITEM_TYPE_NAMES[1] else False
    ...
    def unicName(self):
        ...
        """
        return self.descriptor.type.name if self.itemTypeName == ITEM_TYPE_NAMES[1] else self.descriptor['name']
    ...
    def name(self):
        ...
        """
        return self.descriptor.type.userString if self.itemTypeName == ITEM_TYPE_NAMES[1] else self.descriptor['userString']
    ...
    def shortName(self):
        ...
        """
        return self.descriptor.type.shortUserString if self.itemTypeName == ITEM_TYPE_NAMES[1] else self.descriptor['shortUserString']
    ...
    def tags(self):
        ...
        Set of tags
        """
        return self.descriptor.type.tags if self.itemTypeName == ITEM_TYPE_NAMES[1] else self.descriptor['tags']
    ...
    def icon(self):
        ...
        return _ICONS_MASK[:-4] % {'type': self.itemTypeName,
         'subtype': '',
         'unicName': self.descriptor['icon'][0]} if self.itemTypeName == ITEM_TYPE_NAMES[10] else _ICONS_MASK % {'type': self.itemTypeName,
         'subtype': '',
    ...
    def __eq__(self, other):
        if other is None:
            ...
            return self.descriptor.type.id == other.descriptor.type.id if self.itemTypeName == ITEM_TYPE_NAMES[1] else self.compactDescr == other.compactDescr
...
class InventoryItem(FittingItem):
    ...
    def nation(self):
        ...
        Nation index from nations.INDICES
        """
        return self.descriptor.type.id[0] if self.itemTypeName == ITEM_TYPE_NAMES[1] else self.descriptor['id'][0]
    ...
    def target(self):
        ...
        """
        return 1 if self.isCurrent else 2
...
class InventoryVehicle(InventoryItem):
    ...
    def __getHealth(self):
        ...
        return self.__health if self.__health > 0 else 0
    ...
    def modelState(self):
        if self.__health < 0:
            return 'exploded'
        return 'destroyed' if self.repairCost > 0 and self.health == 0 else 'undamaged'

    @property
    def isSellPosible(self):
        return True if self.repairCost == 0 and not self.lock else False
    ...
    def getStateLevel(self):
        ...
        return InventoryVehicle.STATE_LEVEL.INFO if state in ('undamaged',) else InventoryVehicle.STATE_LEVEL.WARNING
    ...
    def __eq__(self, other):
        return self.inventoryId == other.inventoryId if other is not None and isinstance(other, InventoryVehicle) else False
...
class InventoryTankman(InventoryItem):
    ...
    def __eq__(self, other):
        return self.inventoryId == other.inventoryId if other is not None and isinstance(other, InventoryTankman) else False
...
def getItemByCompact(compact):
    return FittingItem.unpack(compact) if compact else None
