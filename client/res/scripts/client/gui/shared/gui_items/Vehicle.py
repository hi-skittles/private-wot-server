...
class Vehicle(FittingItem, HasStrCD):
    ...
    def rentExpiryTime(self):
        return self.rentInfo[0] if self.rentInfo is not None else 0

    @property
    def rentCompensation(self):
        return self.rentInfo[1] if self.rentInfo is not None else (0, 0)
    ...
    def isOnlyForEventBattles(self):
        ...
        return self._checkForTags(eventBattles.vehicleTags) if eventBattles is not None else False
