...
# Compiled at: 2015-02-12 09:05:01
...
class FortPermissions(interfaces.IFortPermissions):
    ...
    def _checkPermission(self, cmd):
        ...
        return False if cmdRoles > 0 and self._roles & cmdRoles == 0 else True
...
class IntroFortLimits(interfaces.IFortLimits):
    ...
    def isCreationValid(self):
        return (False, FORT_RESTRICTION.CREATION_MIN_COUNT) if self.getClanMembersCount() < fortified_regions.g_cache.clanMembersForStart else (True, '')
    ...
    def isDirectionValid(self, direction, open=True):
        if direction < 1:
            ...
            return (False, FORT_RESTRICTION.DIRECTION_NOT_ENOUGH_MEMBERS) if open and self.getDirectionsMembersRequirements().get(openedDirections + 1, 0) > self.getClanMembersCount() else (True, '')

    def canBuild(self, buildingID):
        fort = getClientFort()
        return (False, FORT_RESTRICTION.UNKNOWN) if fort is None else (True, '')
...
class FortLimits(IntroFortLimits):
    ...
    def isOrderValid(self, orderTypeID, count=1, add=True):
        ...
        if fort is None:
            ...
            if order is None:
                return (False, FORT_RESTRICTION.UNKNOWN)
            return (False, FORT_RESTRICTION.ORDER_MAX_COUNT) if add and order.count + count > order.maxCount else (True, '')
    ...
    def isSortieCreationValid(self, level=None):
        ...
        if fort is None:
            ...
            return (False, FORT_RESTRICTION.SORTIE_HAS_MODAL_ENTITY) if not state.isInUnit(PREBATTLE_TYPE.SORTIE) and state.hasModalEntity else (True, '')
