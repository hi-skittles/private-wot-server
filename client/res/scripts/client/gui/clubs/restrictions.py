...
# Compiled at: 2015-04-02 08:19:48
import operator
...
class AccountClubLimits(RestrictionsCollection, interfaces.IAccountClubLimits):
    ...
    def canRevokeApplication(self, profile, club=None):
        stateID = profile.getState().getStateID()
        return error(_CCR.NOT_IN_APPLICANTS) if stateID != _CCB.SENT_APP or profile.getState().getClubDbID() != club.getClubDbID() else self._isClubRequestValid(_CRT.REVOKE_APPLICATION, club, 'canRevokeApplication')
    ...
    def canLeaveClub(self, profile, club=None):
        stateID = profile.getState().getStateID()
        return error(_CCR.HAS_NO_CLUB) if stateID != _CCB.HAS_CLUB else self._isClubRequestValid(_CRT.LEAVE_CLUB, club, 'canLeaveClub')
    ...
    def canChangeWebSettings(self, profile, club=None):
        ...
        return error(_CCR.DEFAULT) if not permissions.canChangeWebSettings() else success()
