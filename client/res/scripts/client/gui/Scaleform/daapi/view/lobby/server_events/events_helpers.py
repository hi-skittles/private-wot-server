...
class _ActionInfo(_EventInfo):
    ...
    def _getActiveDateTimeString(self):
        return formatters.formatYellow(QUESTS.DETAILS_HEADER_COMETOEND) if self.event.getFinishTimeLeft() <= GUI_SETTINGS.actionComeToEnd else super(_ActionInfo, self)._getActiveDateTimeString()

    def _getTimerMsg(self):
        return makeHtmlString('html_templates:lobby/quests', 'comeToEnd') if self.event.getFinishTimeLeft() <= GUI_SETTINGS.actionComeToEnd else super(_ActionInfo, self)._getTimerMsg()
