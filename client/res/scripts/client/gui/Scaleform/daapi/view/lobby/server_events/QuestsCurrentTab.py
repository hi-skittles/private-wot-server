...
# Compiled at: 2014-11-14 10:13:39
import constants
...
class QuestsCurrentTab(QuestsCurrentTabMeta):
    ...
    def getQuestInfo(self, eventID):
        ...
        return events_helpers.getEventDetails(event, svrEvents) if event is not None else None
    ...
    def __filterFunc(self, a):
        if self.__filterType == self.FILTER_TYPE.ACTIONS and not self._isAction(a):
            return False
        return False if self.__filterType == self.FILTER_TYPE.QUESTS and not self._isQuest(a) else not self.__hideCompleted or not a.isCompleted()
