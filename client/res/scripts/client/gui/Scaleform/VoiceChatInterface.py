...
class _VoiceChatInterface(UIInterface):
    ...
    def getPlayerDBID(self):
        ...
        if type(BigWorld.player()) is Account.PlayerAccount:
            ...
            return BigWorld.player().arena.vehicles[BigWorld.player().playerVehicleID].get('accountDBID', None) if type(BigWorld.player()) is Avatar.PlayerAvatar and hasattr(BigWorld.player(), 'playerVehicleID') else None
